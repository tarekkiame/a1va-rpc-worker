...
app.post('/start-session', async (req, res) => {
  ...
  if (!sessionId || !avatarId || !livekitUrl || !livekitToken || !supabaseUrl) {
    return res.status(400).json({
      status: 'error',
      error: 'missing_required_fields',
      required: ['sessionId', 'avatarId', 'livekitUrl', 'livekitToken', 'supabaseUrl'],
    });
  }
  ...
  startSession({ sessionId, avatarId, livekitUrl, livekitToken, supabaseUrl }).catch((err) => {
